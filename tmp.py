...
for depth in range(0, 100):
    ...
    a = tf.random.normal([1, 27, 27, 384], 0, 255, tf.float32, seed=1)
    ...
    array.append(
        (duration, board_power / float(count), cpu_power / float(count), ram_power / float(count),
         gpu_power / float(count)))
    reset()
    mutex.release()
    """
print("duration,board,cpu,ram,gpu")
for i in range(len(array)):
    print("{},{},{},{},{}".format(array[i][0], array[i][1], array[i][2], array[i][3], array[i][4]))
